    ...
    public Map<String, PopulasjonstilgangResultat> kontroller(String navident, Set<PopulasjonstilgangRequest> foresporsler) {
        try {
            ...
            return populasjonstilgangResponse.resultater().stream().map(r -> switch(RestResponse.Status.fromStatusCode(r.status())) {
                ...
                case RestResponse.Status.NOT_FOUND -> new PopulasjonstilgangResultat(false,  r.personident(), new PopulasjonstilgangResultat.TilgangAvvist(AVVIST_PERSON_UKJENT, "Kunne ikke identifisere person"));
                case RestResponse.Status.FORBIDDEN -> new PopulasjonstilgangResultat(false, r.personident(),  new PopulasjonstilgangResultat.TilgangAvvist(PopulasjonstilgangResultat.Kode.valueOf(r.detaljer().kode()), r.detaljer().begrunnelse()));
                default -> {
